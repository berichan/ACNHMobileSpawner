...
public class UI_ACItemGrid : MonoBehaviour
{
	...
	{
		AutoInjector autoInjector = injector;
		uint offset = sbc.GetDefaultOffset();
		autoInjector.SetWriteOffset(in offset);
		try
		{
			InjectionResult injectionResult = injector.Read(force: true);
	...
	public void WriteToSource()
	{
		...
		try
		{
			InjectionResult injectionResult = injector.Write(force: true);
			if (injectionResult == InjectionResult.Success)
			{
